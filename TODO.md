# Plan de développement - Fonctionnalités restantes

## 1. Système d'annonces ✅ EN COURS
- [x] Créer le composant AnnouncementsManagement pour admins/enseignants
- [x] Créer le composant AnnouncementsView pour consulter les annonces
- [x] Mettre à jour MainApp pour gérer la vue annonces
- [x] Tester la fonctionnalité

## 2. Messages privés
- [ ] Créer le composant MessagesManagement pour la messagerie
- [ ] Créer le composant MessageCompose pour rédiger des messages
- [ ] Créer le composant MessageView pour lire les messages
- [ ] Mettre à jour MainApp pour gérer les vues messages

## 3. Gestion des cours
- [ ] Créer le composant CoursesManagement pour les enseignants
- [ ] Créer le composant CoursesView pour les élèves
- [ ] Créer le composant CourseDetail pour voir le contenu d'un cours
- [ ] Mettre à jour MainApp pour gérer les vues cours

## 4. Gestion des devoirs
- [ ] Créer le composant AssignmentsManagement pour les enseignants
- [ ] Créer le composant AssignmentsView pour les élèves
- [ ] Créer le composant AssignmentDetail pour voir un devoir
- [ ] Mettre à jour MainApp pour gérer les vues devoirs

## 5. Gestion des notes
- [ ] Créer le composant GradesManagement pour les enseignants
- [ ] Créer le composant GradesView pour les élèves/parents
- [ ] Créer le composant GradeEntry pour saisir les notes
- [ ] Mettre à jour MainApp pour gérer les vues notes

## 6. Affichage des emplois du temps
- [ ] Créer le composant ScheduleView pour afficher l'emploi du temps
- [ ] Créer le composant ScheduleManagement pour les admins
- [ ] Mettre à jour MainApp pour gérer la vue emploi du temps
